...
varType rand01() {
  ...
  thread_local std::uniform_real_distribution<varType> dist(varType(0),
                                                            varType(1));
  return dist(rng);
}
...
void Particles::InitParticleGrid(const Fields2D &fields) {
  ...
  const std::size_t cap = static_cast<std::size_t>(fluidCells) * ppcx * ppcy;
    ...
    for (int icell = 0; icell < nx; ++icell) {
      ...
      for (int a = 0; a < ppcx; ++a) {
        for (int b = 0; b < ppcy; ++b) {
          ...
          if (needsAffine) {
            cu_x.push_back(varType(0));
            cu_y.push_back(varType(0));
            cv_x.push_back(varType(0));
            cv_y.push_back(varType(0));
          }
        }
      }
    }
