...
void PIC::Advect() {
    ...
    const auto clampToLastFluidPoint =
        [&](const varType x0, const varType y0, const varType x1,
            const varType y1, varType &xc, varType &yc) {
    ...
    for (int ci = 0; ci < nx; ++ci) {
        for (int cj = 0; cj < ny; ++cj) {
            ...
            while (local_idx < n) {
                ...
                const int ci1 = std::clamp(static_cast<int>(std::floor(xAdv / dx)), 0, nx - 1);
                const int cj1 = std::clamp(static_cast<int>(std::floor(yAdv / dy)), 0, ny - 1);
