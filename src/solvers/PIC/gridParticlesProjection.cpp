...
// ScatterToGrid: kept for FLIP / APIC subclasses that build a velocity
// delta on u_sum/u_weight.  Not used by base-PIC projection anymore.
...
// scatterCell: kept for FLIP / APIC subclasses.  Not called by base PIC.
...

        {
            ...
            for (int8_t dj = -radius; dj <= radius; ++dj) {
                for (int8_t di = -radius; di <= radius; ++di) {
                    ...
                    const varType k = hat(xg - varType(i)) * hat(yg - varType(j));
                    if (k > varType(0)) {
                        OMP_PRAGMA(omp atomic) u_sum(i, j)    += k * up;
                        OMP_PRAGMA(omp atomic) u_weight(i, j) += k;
                    }
                }
            }
        }
        {
            ...
            for (int8_t dj = -radius; dj <= radius; ++dj) {
                for (int8_t di = -radius; di <= radius; ++di) {
                    ...
                    const varType k = hat(xg - varType(i)) * hat(yg - varType(j));
                    if (k > varType(0)) {
                        OMP_PRAGMA(omp atomic) v_sum(i, j)    += k * vp;
                        OMP_PRAGMA(omp atomic) v_weight(i, j) += k;
                    }
                }
            }
        }
