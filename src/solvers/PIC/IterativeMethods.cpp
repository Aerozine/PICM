...
double PIC::computeResidualNorm(const varType coef) const {
...
void PIC::SolveJacobi(int maxIters, double tol) {
...
void PIC::SolveGaussSeidel(int maxIters, double tol) {
  const varType coef = density * dx * dx / dt;
  fields->Div();
...
void PIC::SolveRedBlackGaussSeidel(int maxIters, double tol) {
  const varType coef = density * dx * dx / dt;
  fields->Div();
