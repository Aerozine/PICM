#!/bin/bash -l
#SBATCH --job-name=lamb-apic
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=01:00:00
#SBATCH --output=lamb-apic_%j.out
#SBATCH --error=lamb-apic_%j.err

set -euo pipefail

if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Error: submit this script with sbatch, for example:"
  echo "  sbatch PICM/test/CH6/section-6-3/.launch/run-lamb-apic.sh"
  exit 1
fi
...
CONFIG="${CONFIG:-test/CH6/section-6-3/lamb-apic.json}"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo "  sbatch --export=ALL,CONFIG=test/CH6/section-6-3/lamb-apic.json $0" >&2
  exit 1
fi
