#!/bin/bash -l
#SBATCH --job-name=manometer-flip
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --time=00:30:00
#SBATCH --output=manometer-flip_%j.out
#SBATCH --error=manometer-flip_%j.err

set -euo pipefail

if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Error: submit this script with sbatch, for example:"
  echo "  sbatch PICM/test/tension/section-8-8-3/.launch/run-manometer-flip.sh"
  exit 1
fi
...
CONFIG="${CONFIG:-test/FLAPIC/manometer/manometer-flip.json}"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo "  sbatch --export=ALL,CONFIG=test/FLAPIC/manometer/manometer-flip.json $0" >&2
  exit 1
fi
