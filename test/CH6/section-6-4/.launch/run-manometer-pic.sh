#!/bin/bash -l
#SBATCH --job-name=manometer-pic
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=02:00:00
#SBATCH --output=manometer-pic_%j.out
#SBATCH --error=manometer-pic_%j.err

set -euo pipefail

if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Error: submit this script with sbatch, for example:"
  echo "  sbatch PICM/test/tension/section-8-8-3/.launch/run-manometer-pic.sh"
  exit 1
fi
...
CONFIG="${CONFIG:-test/FLAPIC/manometer/manometer-pic.json}"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo "  sbatch --export=ALL,CONFIG=test/FLAPIC/manometer/manometer-pic.json $0" >&2
  exit 1
fi
