#!/bin/bash -l
#SBATCH --job-name=manometer-apic
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --time=00:30:00
#SBATCH --output=manometer-apic_%j.out
#SBATCH --error=manometer-apic_%j.err

set -euo pipefail

if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Error: submit this script with sbatch, for example:"
  echo "  sbatch PICM/test/tension/section-8-8-3/.launch/run-manometer-apic.sh"
  exit 1
fi
...
CONFIG="${CONFIG:-test/FLAPIC/manometer/manometer-apic.json}"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo "  sbatch --export=ALL,CONFIG=test/FLAPIC/manometer/manometer-apic.json $0" >&2
  exit 1
fi
