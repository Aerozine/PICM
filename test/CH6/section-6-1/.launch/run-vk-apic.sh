#!/bin/bash -l
#SBATCH --job-name=vk-apic
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=01:00:00
#SBATCH --output=vk-apic_%j.out
#SBATCH --error=vk-apic_%j.err

set -euo pipefail

if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Error: submit this script with sbatch, for example:"
  echo "  sbatch PICM/test/CH6/section-6-1/.launch/run-vk-apic.sh"
  exit 1
fi
...
CONFIG="${CONFIG:-test/CH6/section-6-1/vk-apic.json}"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo "  sbatch --export=ALL,CONFIG=test/CH6/section-6-1/vk-apic.json $0" >&2
  exit 1
fi
