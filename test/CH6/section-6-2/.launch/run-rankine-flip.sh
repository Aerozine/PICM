#!/bin/bash -l
#SBATCH --job-name=rankine-flip
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=02:00:00
#SBATCH --output=rankine-flip_%j.out
#SBATCH --error=rankine-flip_%j.err

set -euo pipefail

if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Error: submit this script with sbatch, for example:"
  echo "  sbatch PICM/test/CH6/section-6-2/.launch/run-rankine-flip.sh"
  exit 1
fi
...
CONFIG="${CONFIG:-test/CH6/section-6-2/rankine-flip.json}"
...
if [ ! -f "$CONFIG" ]; then
  ...
  echo "  sbatch --export=ALL,CONFIG=test/CH6/section-6-2/rankine-flip.json $0" >&2
  exit 1
fi
